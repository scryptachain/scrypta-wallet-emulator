...
export default {
  ...
  methods: {
    ...
    processCommand(command){
      ...
      return new Promise(async response => {
        let res = await app.axios.post('http://localhost:4498', {command: command})
  ...
  mounted () {
    ...
    const commandEmitter = (commandText) => {
      let prms = new Promise(async (response) => {
        app.waiting = true
        var data = commandText
        if(data === undefined){
          let input = document.getElementsByClassName('input')
          data = input[0].innerHTML
          setTimeout(async function(){
            let res = await app.processCommand(data)
            app.terminal.echo(res)
          },5)
        }
        let res = await app.processCommand(data)
        response(res)
    ...
    var $ptty = $('#terminal', '.vue-terminal-wrapper').Ptty({
      i18n: {
        welcome: 'Welcome on Scrypta Wallet CLI, a simple bridge to our lyra-cli to test and learn how our wallet works.'
      },
      ps: this.consoleSign,
      allowArbitrary: true,
...
<style lang="css">
.vue-terminal-wrapper{ width:100%; height:100%; position:fixed; top: 0; left:0;}
/* hight settings */
